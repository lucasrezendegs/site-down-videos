      ...
      <div className="rounded-2xl bg-gradient-to-br from-amber-950/40 via-zinc-900 to-zinc-900 border border-amber-500/30 p-5 sm:p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end">
            <button
              ...
              className="px-4 py-3 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-medium text-xs flex items-center gap-2 border border-zinc-700 transition-colors"
            >
              {isPlayingAudio ? (
                <>
              ...
              >
                {downloadingId === activeFormat.id ? (
                  ...
                  <>
                    ...
                    <span>Baixado! Salvar de novo</span>
                  </>
                ) : (
                  <>
        ...
        <div className="mt-5 pt-4 border-t border-zinc-800/80 flex items-center gap-1.5 h-10 px-2 bg-zinc-950/40 rounded-xl overflow-hidden">
          {Array.from({ length: 48 }).map((_, i) => {
            ...
            const isHighlighted = isPlayingAudio ? (i % 6 === 0 ? 'bg-amber-400' : 'bg-amber-500/70') : 'bg-zinc-700';

            return (
              <div
                key={i}
                className={`flex-1 rounded-full transition-all duration-300 ${isHighlighted}`}
                style={{
                  height: isPlayingAudio ? `${Math.max(15, (baseH * (Math.sin(i + Date.now() / 200) + 1.2)) / 2)}%` : `${baseH * 0.5}%`,
                }}
              />
            );
          })}
                ...
                >
                  {isDownloading ? (
                    ...
                    <>
                      ...
                      <span>Baixar {fmt.extension.toUpperCase()}</span>
                    </>
                  )}
                </button>
              </div>
